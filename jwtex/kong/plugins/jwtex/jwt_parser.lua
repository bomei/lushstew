-- jwtex verification module
-- Adapted version of x25/luajwtex for Kong. It provides various improvements and
-- an OOP architecture allowing the jwtex to be parsed and verified separatly,
-- avoiding multiple parsings.
--
-- @see https://github.com/x25/luajwtex
...
local asn_sequence = require "kong.plugins.jwtex.asn_sequence"
...
-- Used to separate the header, claims and signature part of a jwtex
...
--- Parse a jwtex
-- Parse a jwtex and validate header values.
-- @param token jwtex to parse
...
local function decode_token(token)
    ...
    if header.typ and header.typ:upper() ~= "jwtex" then
...
local function encode_token(data, key, alg, header)
    ...
    local header = header or { typ = "jwtex", alg = alg }
    local segments = {
...
--- Instanciate a jwtex parser
-- Parse a jwtex and instanciate a jwtex parser for further operations
...
--- Verify a jwtex signature
-- Verify the current jwtex signature against a given key
